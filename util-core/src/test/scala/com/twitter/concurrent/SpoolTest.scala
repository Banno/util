...
class SpoolTest extends WordSpec {
  ...
  "Simple resolved Spool" should {
    ...
    "take" in {
      ...
      Await.result(ls.take(2).toSeq) === Seq(1,2)
      Await.result(ls.take(1).toSeq) === Seq(1)
      Await.result(ls.take(0).toSeq).isEmpty === true
      Await.result(ls.take(-2).toSeq).isEmpty === true
    }
  }
